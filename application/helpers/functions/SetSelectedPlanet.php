...
function SetSelectedPlanet ( /*&$CurrentUser*/ ) {
	...
	if (isset($SelectPlanet)      &&
		...
		isset($RestorePlanet)     &&
		$RestorePlanet == 0) {
		$IsPlanetMine   = fs()->doquery("SELECT `id` FROM {{table}} WHERE `id` = '". $SelectPlanet ."' AND `id_owner` = '". $CurrentUser['id'] ."';", 'planets', true);
		if ($IsPlanetMine) {
			...
			fs()->doquery("UPDATE {{table}} SET `current_planet` = '". $SelectPlanet ."' WHERE `id` = '".$CurrentUser['id']."';", 'users');
		}
	}
}
