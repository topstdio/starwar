...
function AddBuildingToQueue ( /*&$CurrentPlanet,*/ $CurrentUser, $Element, $AddMode = true) {
	...
	if ( $QueueID != false ) {
		...
		if ($InArray != 0) {
			$ActualLevel  = $CurrentPlanet[$ge_resource[$Element]];
			if ($AddMode == true) {
				$BuildLevel   = $ActualLevel + 1 + $InArray;
				$CurrentPlanet[$ge_resource[$Element]] += $InArray;
				$BuildTime    = fs()->GetBuildingTime($CurrentUser, $CurrentPlanet, $Element);
				$CurrentPlanet[$ge_resource[$Element]] -= $InArray;
			} else {
				$BuildLevel   = $ActualLevel - 1 + $InArray;
				$CurrentPlanet[$ge_resource[$Element]] -= $InArray;
				$BuildTime    = fs()->GetBuildingTime($CurrentUser, $CurrentPlanet, $Element) / 2;
				$CurrentPlanet[$ge_resource[$Element]] += $InArray;
			}
		} else {
			$ActualLevel  = $CurrentPlanet[$ge_resource[$Element]];
