...
function HandleTechnologieBuild( /*&$CurrentPlanet, &$CurrentUser */)
{
    ...
    if ($CurrentUser['b_tech_planet'] != 0) {
        ...
        if ($CurrentUser['b_tech_planet'] != $CurrentPlanet['id']) {
            ...
            $WorkingPlanet = fs()->doquery("SELECT * FROM {{table}} WHERE `id` = '" . $CurrentUser['b_tech_planet'] . "';", 'planets', true);
        }

        if (!empty($WorkingPlanet)) {
        ...
        if ($ThePlanet['b_tech'] <= time() &&
            ...
            $CurrentUser[$ge_resource[$ThePlanet['b_tech_id']]]++;
            ...
            $QryUpdateUser .= "`" . $ge_resource[$ThePlanet['b_tech_id']] . "` = '" . $CurrentUser[$ge_resource[$ThePlanet['b_tech_id']]] . "', ";
            ...
            fs()->doquery("UPDATE {{table}} SET `b_tech_planet` = '0'  WHERE `id` = '" . $CurrentUser['id'] . "';", 'users');
            $Result['WorkOn'] = "";
            $Result['OnWork'] = false;
