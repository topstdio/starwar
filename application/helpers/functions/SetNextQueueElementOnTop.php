    ...
    if ($CurrentPlanet['b_building'] == 0) {
        ...
        if ($CurrentQueue != 0) {
            ...
            while ($Loop == true) {
                ...
                $HaveRessources = fs()->IsElementBuyable($CurrentUser, $CurrentPlanet, $Element, true, $ForDestroy);
                if ($ForDestroy) {
                    if ($CurrentPlanet[$ge_resource[$Element]] == 0) {
                        $HaveRessources = false;
                        $HaveNoMoreLevel = true;
                    }
                }
                if ($HaveRessources == true) {
                    $Needed = fs()->GetBuildingPrice($CurrentUser, $CurrentPlanet, $Element, true, $ForDestroy);
                    ...
                    if ($HaveNoMoreLevel == true) {
                        ...
                        $Message = sprintf($lang['sys_notenough_money'], $ElementName,
                            fs()->pretty_number($CurrentPlanet['metal']), $lang['Metal'],
                            fs()->pretty_number($CurrentPlanet['crystal']), $lang['Crystal'],
                            fs()->pretty_number($CurrentPlanet['deuterium']), $lang['Deuterium'],
                            fs()->pretty_number($Needed['metal']), $lang['Metal'],
                            fs()->pretty_number($Needed['crystal']), $lang['Crystal'],
                            fs()->pretty_number($Needed['deuterium']), $lang['Deuterium']);
                    }

                    fs()->SendSimpleMessage($CurrentUser['id'], '', '', 99, $lang['sys_buildlist'], $lang['sys_buildlist_fail'], $Message);
